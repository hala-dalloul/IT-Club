...
function harness(responder) {
  ...
  function load(path) {const exports={};const js=ts.transpileModule(readFileSync(path,'utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2022}}).outputText;
    vm.runInNewContext(js,{exports,AbortSignal,fetch:async(url,options)=>{calls.push({url,options});

return responder(url,options);},require(name){if(name==='./model')return load('src/lib/club/model.ts');

if(name==='./supabase')return {supabase(){authCalls++;

return {auth:{getSession:async()=>({data:{session:{access_token:'test-session'}},error:null})}};}};
