...
function harness(kind = "join") {
  ...
  function sheet() {const rows=[];

return {getLastRow:()=>rows.length,setFrozenRows(){},setRightToLeft(){},getRange(row,col,n,width){const range={setValues(values){values.forEach((v,i)=>{rows[row-1+i]??=[];v.forEach((x,j)=>rows[row-1+i][col-1+j]=x);});

return range;},setRichTextValues(values){return range.setValues(values.map(r=>r.map(v=>v.text)));},setFontWeight(){return range;},setWrap(){return range;},getValues(){return Array.from({length:n},(_,i)=>Array.from({length:width},(_,j)=>rows[row-1+i]?.[col-1+j]??''));},getDisplayValues(){return range.getValues().map(r=>r.map(String));}};

return range;}};}

  const context=vm.createContext({ContentService:{MimeType:{JSON:'json'},createTextOutput(text){return {setMimeType(){return JSON.parse(text);}};}},PropertiesService:{getScriptProperties(){return {getProperty:k=>props.get(k),setProperty(k,v){props.set(k,v);}};}},LockService:{getScriptLock(){return {tryLock(){assert.equal(held,false);held=true;

return true;},releaseLock(){held=false;}};}},SpreadsheetApp:{openById(id){if(!books.has(id)) books.set(id,new Map());const b=books.get(id);

return {getSheetByName:n=>b.get(n),insertSheet(n){const s=sheet();b.set(n,s);

return s;}};},newRichTextValue(){return {setText(text){return {build:()=>({text})};}};},flush(){}},UrlFetchApp:{fetch(){throw Error('Unexpected authentication call');}}});

  vm.runInContext(readFileSync('integrations/google-sheets/Code.gs','utf8'),context);
  props.set("FORM_KIND", kind);
  context.setup();

return {context,props,post:b=>context.doPost({postData:{contents:JSON.stringify(b)}})};
}
