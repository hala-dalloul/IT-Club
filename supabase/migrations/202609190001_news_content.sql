-- Separate news from events by persisted kind. Existing rows remain unchanged.
begin;
...
alter table public.club_content drop constraint if exists club_content_kind_check;
alter table public.club_content add constraint club_content_kind_check
  check(kind in ('projects','members','events','news','achievements','partners'));
commit;
