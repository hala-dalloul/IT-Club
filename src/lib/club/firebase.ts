...
export function watchContent(
  ...
  next: (rows: Content[]) => void,
  fail: (error: Error) => void,
) {
  return onSnapshot(
    query(collection(firebase().db, name), orderBy("updatedAt", "desc")),
    (snap) => next(snap.docs.map((d) => ({ ...d.data(), id: d.id }) as Content)),
    fail,
  );
}
export async function saveContent(
  ...
  await setDoc(target, { ...value, updatedBy: auth.currentUser.uid, updatedAt: serverTimestamp() });
}
export async function removeContent(name: ContentCollection, id: string) {
  await deleteDoc(doc(firebase().db, name, id));
}
export async function submitForm(
  name: "joinRequests" | "contactMessages",
  data: Record<string, string>,
) {
  await addDoc(collection(firebase().db, name), {
    ...data,
    submittedAt: serverTimestamp(),
    ...(name === "joinRequests" ? { status: "new" } : { isRead: false }),
  });
}
export async function saveSettings(settings: Settings) {
  await setDoc(doc(firebase().db, "settings", "public"), settings);
}
